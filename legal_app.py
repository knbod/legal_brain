...
pure_white_css = """
    <style>
    /* A. HIDE STREAMLIT UI ELEMENTS (The "Nuclear" Option) */
    ...
    /* B. FORCE WHITE BACKGROUNDS EVERYWHERE */
    .stApp {
        background-color: #FFFFFF;
        color: #333333;
    }
    
    /* C. INPUT FIELDS (Make them White with Grey Border) */
    ...
    /* E. BUTTONS (Ocean Green for Primary, White for Secondary) */
    button[kind="primary"] {
...
"""
st.markdown(pure_white_css, unsafe_allow_html=True)
...
if st.session_state["user"] is None:
    ...
    with st.sidebar:
        ...
        st.divider()
        # Secondary button (White with Grey Border)
    ...
    with tab_dash:
        ...
        if res.data:
            ...
            m4.metric("🔴 Action", len(df[df["Status"].isin(["EXPIRED", "MISSING"])]))
            ...
            # NOTE: Tables will now be white automatically due to CSS
            with sub_safe:
                st.dataframe(df[df["Status"] == "SAFE"][["name", "trade", "insurance_expiry_date"]], use_container_width=True)
            with sub_warn:
                st.warning(f"Expires in < {safe_days} days.")
    ...
    with tab_import:
        ...
        if uploaded_file:
            ...
            text_cols = df_imp.select_dtypes(include=['object', 'string']).columns.tolist() or df_imp.columns.tolist()
            all_cols = df_imp.columns.tolist()

            c_name_col, c_date_col = st.columns(2)
            with c_name_col:
                name_col = st.selectbox("Select Name Column", text_cols)
            with c_date_col:
                date_options = [c for c in all_cols if c != name_col]
                date_col = st.selectbox("Select Expiry Column", date_options)
            
            if name_col == date_col:
                st.error("Name and Date cannot be the same.")
            else:
                if st.button("Run Import", type="primary"):
                    ...
                    exist_set = {r['name'] for r in existing.data} if existing.data else set()
                    ...
                    for i, row in df_imp.iterrows():
                        ...
                        if not nm or nm.lower() == "nan" or nm in exist_set: 
                            skip += 1
                            continue
                    ...
                    st.success(f"✅ Added {add}, Skipped {skip}")
                    time.sleep(1)
                    st.rerun()
    ...
    with tab_audit:
        ...
        if res.data:
            ...
            sel_wk = st.selectbox("Select Worker:", list(w_map.keys()))
            sel_id = w_map[sel_wk]
            
            up_file = st.file_uploader("Upload Evidence", type=["png", "jpg", "jpeg"])
            if up_file and st.button("Auto-Extract", type="primary"):
                with st.spinner("Processing..."):
                    ...
                    found = ask_ai_to_read_date(up_file.getvalue(), up_file.type)
                    if len(found) == 10 and found[4] == '-':
                        supabase.table("subcontractors").update({"insurance_expiry_date": found, "data_status": "verified"}).eq("id", sel_id).execute()
                        st.balloons()
                        st.success(f"✅ Fixed: {found}")
                        time.sleep(1)
                        st.rerun()
                    else:
                        st.error(f"AI Failed: {found}")
